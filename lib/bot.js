...
export default class Yunzai extends EventEmitter {
  ...
  async redisExit() {
    if (!(typeof redis === "object" && redis.process))
      return false
    await util.sleep(5000, redis.save())
    return redis.process.kill()
  }
  ...
  async restart() {
    ...
    switch (global.start_type) {
      case "pm2": {
        const ret = await util.exec("pnpm run restart")
    ...
    const ret = spawn(process.argv[0],
      [process.argv[1], "restart", ...process.argv.slice(2)],
