...
import { parseSourceImg, url2Base64 } from "../utils/getImg.js";
...
// 使机器人可以对其第一人称回应
const reg_chatgpt_for_firstperson_call = new RegExp(
  Config.getConfig()?.botName ||
    `sf-plugin-bot-name-${Math.floor(10000 + Math.random() * 90000)}`,
  "g"
);
...
export class SF_Painting extends plugin {
  ...
  get_use_sf_key(config_date) {
    ...
    while (!use_sf_key && count < config_date.sf_keys.length) {
      ...
      if (this.sf_keys_index < config_date.sf_keys.length - 1) {
        this.sf_keys_index++;
      } else this.sf_keys_index = 0;

      if (config_date.sf_keys[this.sf_keys_index].isDisable) continue;
      else {
  ...
  get_use_ggKey(config_date) {
    ...
    const keysArr = config_date.ggKey
      .split(/[,，]/)
      .map((key) => key.trim())
      .filter(Boolean);
    ...
    this.currentKeyIndex_ggKey =
      (this.currentKeyIndex_ggKey + 1) % keysArr.length;
  ...
  async sf_setConfig(e) {
    ...
    const match = e.msg.match(
      /^#(sf|SF|siliconflow|硅基流动)设置(画图key|翻译key|翻译baseurl|翻译模型|生成提示词|推理步数|fish发音人|ss图片模式|ggkey|ggbaseurl|gg图片模式|上下文)([\s\S]*)/
    );
    if (match) {
  ...
  async sf_draw(e) {
    ...
    if (config_date.sf_keys.length == 0) {
      await e.reply(
        "请先设置画图API Key。使用命令：#sf设置画图key [值]（仅限主人设置）",
        true
      );
      return false;
    }
    ...
    if (
      config_date.imageModel.match(
        /stabilityai\/stable-diffusion-3-medium|stabilityai\/stable-diffusion-xl-base-1.0|stabilityai\/stable-diffusion-2-1|stabilityai\/stable-diffusion-3-5-large/
      )
    ) {
      canImg2Img = true;
    }
    ...
    if (config_date.generatePrompt) {
      ...
      finalPrompt = await this.generatePrompt(
        userPrompt,
        use_sf_key,
        config_date
      );
    ...
    if (!onleReplyOnce && !config_date.simpleMode) {
      e.reply(
        `@${e.sender.card || e.sender.nickname} ${e.user_id}正在为您生成图片...`
      );
      onleReplyOnce++;
    }
  ...
  async sf_chat(e, chatFinish) {
    ...
    if (config_date.ss_apiBaseUrl) {
      ...
      await e.reply(
        "请先设置API Key。使用命令：#sf设置画图key [值]（仅限主人设置）",
        true
      );
      return true;
    } else {
...
可用别名：#flux绘画
  ...
  async sf_send_pic(
    ...
    try {
      const response = await fetch(
        `${config_date.sfBaseUrl}/image/generations`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${use_sf_key}`,
          ...
          body: JSON.stringify({
            ...
            image: canImg2Img
              ? "data:image/png;base64," + souce_image_base64
              : undefined,
      ...
      if (data?.images?.[0]?.url) {
        ...
        const str_1 = `@${e.sender.card || e.sender.nickname} ${e.user_id}您的${
          canImg2Img ? "图生图" : "文生图"
        }已完成：`;
...
种子：${data.seed}`;
        ...
        if (config_date.simpleMode) {
          const msgx = await common.makeForwardMsg(
            e,
            [str_1, { ...segment.image(imageUrl), origin: true }, str_2, str_3],
            `${e.sender.card || e.sender.nickname} 的${
              canImg2Img ? "图生图" : "文生图"
            }`
          );
          // e.reply(msgx)
  ...
  async gg_chat(e) {
    ...
    let ggBaseUrl =
      config_date.ggBaseUrl || "https://bright-donkey-63.deno.dev";
    ...
    try {
      // 根据是否为图片模式决定是否在消息中显示提取的内容
      const { message: processedMsg, extractedContent: extracted } =
        await processMessageWithUrls(msg, !config_date.gg_useMarkdown);
      msg = processedMsg;
      extractedContent = extracted;
    ...
    let { answer, sources } = await this.generateGeminiPrompt(
      aiMessage,
      ggBaseUrl,
      ggKey,
      config_date,
      ...
      if (useMarkdown) {
        ...
        e.reply(
          await common.makeForwardMsg(
            e,
            forwardMsg,
            `${e.sender.card || e.sender.nickname || e.user_id}的搜索结果`
          )
        );
      } else {
        ...
        if (sources && sources.length > 0) {
          ...
          e.reply(
            await common.makeForwardMsg(
              e,
              sourceMsg,
              `${e.sender.card || e.sender.nickname || e.user_id}的搜索来源`
            )
          );
        }
      }
  ...
  async generateGeminiPrompt(
    ...
    const requestBody = {
      systemInstruction: {
        parts: [
          {
            text:
              config_date.gg_Prompt ||
              "你是一个有用的助手，你更喜欢说中文。你会根据用户的问题，通过搜索引擎获取最新的信息来回答问题。你的回答会尽可能准确、客观。",
          },
        ],
      },
      contents: [],
    ...
    try {
      const response = await fetch(
        `${ggBaseUrl}/v1beta/models/${
          config_date.gg_model || "gemini-2.0-flash-exp"
        }:generateContent?key=${ggKey}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
