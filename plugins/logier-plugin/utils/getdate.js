...
export function getFunctionData(YamlName, ArrayName, Function) {
  ...
  const functionData =
    Config[ArrayName].find((item) => item.FunctionName === Function) ||
    Config[ArrayName].find((item) => item.FunctionName === "default");
  return functionData;
}
...
export async function getRandomImage(category) {
  ...
  if (["pc", "mb", "sq"].includes(category)) {
    ...
    const allFiles = await Promise.all(
      ["pc", "mb", "sq"].map(readCategoryFiles)
    );
...
export async function getemoji(e, category) {
  ...
  try {
    ...
    const groupData =
      BlackList.find((item) => String(item.group) === String(e.group_id)) ||
      BlackList.find((item) => item.group === "default");
    ...
    if (!exclude.includes(category)) {
      if (category === "表情包仓库") {
        if (Math.random() < Number(EmojiConfig.CustomerRate)) {
          ...
          let randomValue =
            EmojiIndex[randomKey][
              Math.floor(Math.random() * EmojiIndex[randomKey].length)
            ];
...
export async function getImageUrl(
  imageUrls,
  defaultImageUrl = "./plugins/logier-plugin/resources/gallery/92095127.webp"
) {
