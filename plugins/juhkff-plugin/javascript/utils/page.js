...
export async function generateDailyReport(data) {
    // 定义模板路径和名称
    const templatePath = path.join(PLUGIN_RESOURCES_DIR, "daily_report", "main.html");
    // 定义页面设置
    const viewport = {
        width: 578,
        height: 1885,
    ...
    await page.evaluate((data) => {
        ...
        ${data.data_festival
            .map((fes) => `<p class="moyu-inner">距离<span class="moyu-day-name">${fes[1]}</span>还剩<span class="moyu-day">${fes[0]}</span>天</p>`)
            .join("")}    `;
        // 渲染 B站热点
        ...
        animeContainer.innerHTML = `      ${data.data_anime
            .map((s) => `<div class="anime-border"><img src="${s.image}" class="anime-img"><p class="anime-text">${s.name}</p></div>`)
            .join("")}    `;
        // 渲染60S读世界
        const sixContainer = document.querySelector(".three-border ul");
        sixContainer.innerHTML = `      ${data.data_six
            .map((s) => `<li class="${data.full_show ? "full-show-text" : "normal-text"}">${s}</li>`)
            .join("")}    `;
        // 渲染IT资讯
        ...
        itContainer.innerHTML = `      ${data.data_it
            ...
            .join("")}    `;
        // 渲染今日一言
        document.querySelector(".five-border p").textContent = `“${data.data_hitokoto}”`;
    }, data);
    // 等待图片加载完成
