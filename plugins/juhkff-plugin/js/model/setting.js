...
class Setting {
    ...
    initConfig() {
        ...
        var files = this.getAllFiles(defaultConfigDir).filter((f) => f.endsWith(".yaml"));
        for (const defaultFile of files) {
            ...
            if (!fs.existsSync(path.join(this.configPath, `${relativePath}`))) {
                ...
                fs.copyFileSync(path.join(defaultConfigDir, `${relativePath}`), path.join(this.configPath, `${relativePath}`));
    ...
    getData(folderPath, filename) {
        ...
        try {
            ...
            return YAML.parse(fs.readFileSync(path.join(folderPath, `${filename}.yaml`), "utf8"));
        }
        catch (error) {
